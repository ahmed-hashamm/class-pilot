...
export function TodoItem({ assignment, status, userId, myGroupIds }: TodoItemProps) {
  ...
  return (
    <Link href={`/classes/${assignment.classes?.id}/assignments/${assignment.id}`} className="flex items-stretch group md:transition-all md:duration-300 transition-colors duration-200 [transform:translateZ(0)]
